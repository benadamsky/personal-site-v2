...
const bin =
  process.env.AUDIBLE_BIN || path.join(homedir(), '.local/bin/audible');
// Password for an encrypted auth file (CI); omit locally if the file is plain.
const pw = process.env.AUDIBLE_AUTH_PASSWORD
  ? ['-p', process.env.AUDIBLE_AUTH_PASSWORD]
  : [];
const dir = mkdtempSync(path.join(tmpdir(), 'audible-'));
const raw = path.join(dir, 'library.json');
execFileSync(bin, [...pw, 'library', 'export', '-f', 'json', '-o', raw], {
  stdio: 'inherit'
});
...
const items = JSON.parse(readFileSync(raw, 'utf8'))
  .filter(
    (i) =>
      !(i.genres || '').includes('Podcast') && Number(i.runtime_length_min) > 0
  )
  .map((i) => {
    ...
    return {
      ...
      // date_added is a purchase/library date, not a completion date.
      added: i.date_added
    };
  });
...
items.sort(
  (a, b) => rank[a.status] - rank[b.status] || (b.added > a.added ? 1 : -1)
);
